...
export async function POST(request: Request, context: RouteContext<"/api/apartments/[id]/community/posts">) {
  ...
  const post = await createPost(userId, id, parsed.data);
  if (!post) return NextResponse.json({ message: "이 아파트 커뮤니티 참여 회원만 작성할 수 있습니다." }, { status: 403 });
