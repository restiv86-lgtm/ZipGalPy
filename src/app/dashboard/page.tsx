...
import { homeFeatureHref, selectOwnedHomeId } from "@/lib/home/navigation";
...
export default async function DashboardPage({ searchParams }: PageProps<"/dashboard">) {
  ...
  const [homes, apartmentMemberships, management, expenses] = await Promise.all([listHomes(session.user.id), listUserApartmentMemberships(session.user.id), dashboardManagement(session.user.id), expenseTotals(session.user.id)]);
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={`${styles.container} ${styles.content}`}>
        ...
        <section aria-labelledby="status-title"><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>HOME STATUS</p><h2 id="status-title">우리 집 관리 현황</h2></div></div><div className={styles.statusGrid}>{[["등록된 물건",`${management.countItems}개`],["예정 일정",`${management.countSchedules}개`],["보관 문서",`${management.countDocuments}개`],["수리 기록",`${management.countRepairs}개`]].map(([label,value])=><div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></section>
        {management.upcoming.length>0&&<section aria-labelledby="upcoming-title"><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>UPCOMING</p><h2 id="upcoming-title">다가오는 일정</h2></div></div><div className={styles.homeGrid}>{management.upcoming.map(schedule=>{const day=new Date(schedule.scheduledAt);const today=day.toDateString()===management.now.toDateString();const overdue=day<management.now&&!today;return <article className={styles.homeCard} key={schedule.id}><div><span>{overdue?"지난 미완료 일정":today?"오늘 일정":"7일 이내 일정"}</span><h3>{schedule.title}</h3><p>{schedule.home.name} · {day.toLocaleString("ko-KR")}</p></div><Link href={`/homes/${schedule.homeId}/schedules/${schedule.id}/edit`}>일정 확인 →</Link></article>})}</div></section>}
        <section aria-labelledby="expense-summary-title"><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>HOME EXPENSES</p><h2 id="expense-summary-title">집 관련 지출</h2></div></div><div className={styles.statusGrid}><div><span>이번 달</span><strong>{expenses.month.toLocaleString("ko-KR")}원</strong></div><div><span>올해</span><strong>{expenses.year.toLocaleString("ko-KR")}원</strong></div></div></section>
        <section aria-labelledby="menu-title"><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>SERVICES</p><h2 id="menu-title">집 관리 메뉴</h2>{homes.length > 1 && !selectedHomeId && <p>관리할 집을 선택하면 각 기능으로 바로 이동합니다.</p>}</div></div><div className={styles.menuGrid}><Link href={selectedHomeId ? `/home/${selectedHomeId}/edit` : homes.length === 0 ? "/home/new" : "#homes-title"}><span>🏠</span><strong>집 정보</strong><small>기본 정보와 주소 관리</small></Link><Link href={homeFeatureHref("items", selectedHomeId)}><span>📦</span><strong>우리집 물건</strong><small>물건과 보증 정보 관리</small></Link><Link href={homeFeatureHref("schedule", selectedHomeId)}><span>📅</span><strong>일정</strong><small>점검과 생활 일정 관리</small></Link><Link href={homeFeatureHref("repairs", selectedHomeId)}><span>🔧</span><strong>수리/점검</strong><small>수리와 점검 이력 관리</small></Link><Link href={homeFeatureHref("expenses", selectedHomeId)}><span>💰</span><strong>비용</strong><small>집 관련 지출 관리</small></Link><Link href={homeFeatureHref("contracts", selectedHomeId)}><span>📝</span><strong>계약</strong><small>계약과 만료일 관리</small></Link><Link href={homeFeatureHref("documents", selectedHomeId)}><span>📄</span><strong>문서</strong><small>문서 정보와 만료일 관리</small></Link><Link href="/dashboard/ai"><span>🤖</span><strong>AI 집 관리</strong><small>기능 준비 중</small></Link></div></section>
      </div>
    </main>
  );
}
