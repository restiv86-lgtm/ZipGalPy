  ...
  return (
    <main className={styles.page}>
      ...
      <div className={`${styles.container} ${styles.content}`}>
        ...
        <section aria-labelledby="menu-title"><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>SERVICES</p><h2 id="menu-title">집 관리 메뉴</h2></div></div><div className={styles.menuGrid}><Link href={homes[0] ? `/home/${homes[0].id}/edit` : "/home/new"}><span>🏠</span><strong>집 정보</strong><small>기본 정보와 주소 관리</small></Link><Link href={homes[0]?`/homes/${homes[0].id}/items`:"/home/new"}><span>📦</span><strong>우리집 물건</strong><small>물건과 보증 정보 관리</small></Link><Link href={homes[0]?`/homes/${homes[0].id}/schedules`:"/home/new"}><span>📅</span><strong>일정</strong><small>점검과 생활 일정 관리</small></Link><Link href={homes[0]?`/homes/${homes[0].id}/repairs`:"/home/new"}><span>🔧</span><strong>수리/점검</strong><small>수리와 점검 이력 관리</small></Link><Link href={homes[0]?`/homes/${homes[0].id}/expenses`:"/home/new"}><span>💰</span><strong>비용</strong><small>집 관련 지출 관리</small></Link>{menu.slice(2).filter(([,label])=>label!=="비용").map(([icon,label,path])=><Link href={`/dashboard/${path}`} key={path}><span>{icon}</span><strong>{label}</strong><small>기능 준비 중</small></Link>)}</div></section>
      </div>
    </main>
  );
}
