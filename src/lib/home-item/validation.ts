...
export const homeItemSchema=z.object({name:z.string().trim().min(1).max(120),category:z.enum(categories),brand:nullableText(80),modelName:nullableText(120),purchaseDate:nullableDate,purchasePrice:nullablePrice,warrantyUntil:nullableDate,memo:nullableText(2000),status:z.enum(statuses).default("USING"),createExpense:z.boolean().optional().default(false)});
export type HomeItemInput=Omit<z.infer<typeof homeItemSchema>,"createExpense">&{createExpense?:boolean};
